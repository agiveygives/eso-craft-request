...
export const rawMotifs = (t: TFunction) => [
  {
    value: 'style.none', stone: 'stone.any', label: t('style.none'), color: '#fcb935', isFixed: true,
  },
].concat(
  styles.map((style) => ({
    value: style.name, stone: style.stone, label: t(style.name), color: '#2DC50E', isFixed: true,
  })),
);

const sortedMotifs = (t: TFunction) => [
  {
    value: 'style.none', /* stone: 'stone.any',*/ label: t('style.none'),
  },
].concat(
  styles.sort((a, b) => ((t(a.name) > t(b.name)) ? 1 : -1))
    .map((style) => ({
      value: style.name, /* stone: style.stone,*/ label: t(style.name),
    })),
);
